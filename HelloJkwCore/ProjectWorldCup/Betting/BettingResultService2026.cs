...
public class BettingResultService2026 : IBettingResultService
{
    private readonly MemoryCache _cache2026 = new MemoryCache("Result2026");
    private readonly IFileSystem _fs;

    public BettingResultService2026(
    ...
    {
        if (_cache2026.Contains(nameof(GetGroupStageBettingResultAsync)))
        {
            return (List<WcBettingItem<Team>>)_cache2026[nameof(GetGroupStageBettingResultAsync)];
        }
    ...
    public async Task<List<WcBettingItem<Team>>> GetRound16BettingResultAsync()
    {
        ...
        {
            return (List<WcBettingItem<Team>>)_cache2026[nameof(GetRound16BettingResultAsync)];
        }
    ...
    {
        if (_cache2026.Contains(nameof(GetFinalBettingResultAsync)))
        {
            return (List<WcFinalBettingItem<Team>>)_cache2026[nameof(GetFinalBettingResultAsync)];
        }
    ...
    {
        if (_cache2026.Contains(nameof(GetBettingSummaryAsync)))
        {
            return (List<UserResult>)_cache2026[nameof(GetBettingSummaryAsync)];
        }
    ...
    private static Func<Paths, string> BettingItemDirectoryPath(BettingType bettingType)
    {
        Func<Paths, string> itemPathFunc
            = path => path[WorldCupPath.Betting2026] + $"/{bettingType}";
        return itemPathFunc;
    }
