...
public partial class Betting2026Round16 : JkwPageBase
{
    [Inject] public ISnackbar Snackbar { get; set; }
    [Inject] IWorldCupService WorldCupService { get; set; }
    [Inject] IBettingService BettingService { get; set; }
    [Inject(Key = "2026-round16")] private IBettingRound16Service BettingRound16Service { get; set; }
    ...
    bool AllMatchesAreSetted => Round16Matches.Count == 8
        && Round16Matches.SelectMany(m => m.Teams)
    ...
    private void OpenAiHelperDialog()
    {
        ...
        if (!AllMatchesAreSetted)
        {
            Snackbar.Clear();
            Snackbar.Configuration.PositionClass = Defaults.Classes.Position.TopCenter;
            Snackbar.Add("16강 진출팀이 모두 확정된 후에 사용할 수 있습니다.", Severity.Warning);
            return;
        }
    ...
    private async Task SubmitAiResultAsync()
    {
        ...
        if (!AllMatchesAreSetted)
        {
            SetAiSubmitMessage("16강 진출팀이 모두 확정된 후에 사용할 수 있습니다.", Severity.Error);
            return;
        }
        ...
        try
        {
            var bettingUser = await EnsureRound16BettingJoinedAsync();
            BettingItem = await BettingRound16Service.PickTeamsWithAiAsync(bettingUser, pickedTeams);

            await HideAiHelperDialogAsync(clearInput: true);
            Snackbar.Clear();
            Snackbar.Configuration.PositionClass = Defaults.Classes.Position.TopCenter;
            Snackbar.Add("AI 결과로 8팀을 선택했습니다. (저장되었습니다)", Severity.Success);
        }
        catch (Exception ex)
        {
    ...
    private async Task<BettingUser> EnsureRound16BettingJoinedAsync()
    {
        var bettingUser = await BettingService.GetBettingUserAsync(User);
        if (bettingUser == null)
        {
        ...
        if (!(bettingUser.JoinedBetting?.Contains(BettingType.Round16) ?? false))
        {
            bettingUser = await BettingService.JoinBettingAsync(bettingUser, BettingType.Round16);
        }
        if (!(bettingUser.JoinedBetting?.Contains(BettingType.Round16) ?? false))
    ...
    private bool TryGetAiPickedTeams(out List<Team> pickedTeams, out string errorMessage)
    {
        ...
        using (document)
        {
            ...
            var missingMatches = Enumerable.Range(0, Round16Matches.Count)
            ...
            if (pickedTeams.Count != Round16Matches.Count)
            {
                errorMessage = $"최종 선택은 {Round16Matches.Count}팀이어야 합니다. 현재 {pickedTeams.Count}팀입니다.";
                return false;
            }
        }
        ...
        out string errorMessage)
    {
        ...
        var match = Round16Matches[matchIndex];
    ...
    private bool TryGetMatchIndex(string submittedMatchName, out int matchIndex)
    {
        ...
        return matchIndex >= 0 && matchIndex < Round16Matches.Count;
    }
    ...
    private string BuildAiHelpPrompt()
    {
        if (!AllMatchesAreSetted)
        {
            return "16강 진출팀이 모두 확정된 후에 프롬프트를 만들 수 있습니다.";
        }

        var prompt = new StringBuilder();
        prompt.AppendLine("AI야 월드컵 16강이야. 각 경기에서 8강에 올라갈 것 같은 팀을 한 팀씩 골라줘.");
        ...
        for (var i = 0; i < Round16Matches.Count; i++)
        {
            var match = Round16Matches[i];
        ...
        for (var i = 0; i < Round16Matches.Count; i++)
        {
            var comma = i == Round16Matches.Count - 1 ? "" : ",";
